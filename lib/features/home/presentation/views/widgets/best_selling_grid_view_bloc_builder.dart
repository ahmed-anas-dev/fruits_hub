...
import 'package:fruits_hub/core/widgets/error_builder.dart';
    ...
    return BlocBuilder<ProductsCubit, ProductsState>(
      builder: (context, state) {
        if(state is ProductsLoadedState) {
                  return BestSellingGridView(product: state.products);

          
        }else if(state is ProductsErrorState) {
          return SliverToBoxAdapter(child: ErrorBuilder(text: state.message));
        }else {
          return Skeletonizer.sliver(
            enabled: true,
            
            child:   BestSellingGridView(product: getDummyProdusts()),);
